...
class MedicalHistoryAdmin(admin.ModelAdmin):
    list_display = ('patient', 'condition_name', 'condition_type', 'status', 'onset_date', 'recorded_by')
    list_filter = ('condition_type', 'status', 'recorded_at')
    search_fields = ('patient__patient_number', 'patient__user__first_name', 'patient__user__last_name', 'condition_name', 'icd10_code')
...
class EncounterAdmin(admin.ModelAdmin):
    ...
    search_fields = ('patient__patient_number', 'patient__user__first_name', 'patient__user__last_name', 'doctor__user__first_name', 'doctor__user__last_name')
    ...
    fieldsets = (
        ...
        ('Clinical Information', {
            'fields': ('chief_complaint', 'clinical_notes')
        }),
        ('Timestamps', {
...
class VitalSignAdmin(admin.ModelAdmin):
    list_display = ('encounter', 'temperature', 'systolic_pressure', 'diastolic_pressure', 'recorded_by', 'recorded_at')
    search_fields = ('encounter__patient__patient_number', 'encounter__patient__user__first_name')
    autocomplete_fields = ('encounter', 'recorded_by')

@admin.register(Diagnosis)
class DiagnosisAdmin(admin.ModelAdmin):
    list_display = ('icd10_code', 'description', 'encounter', 'diagnosed_by', 'diagnosed_at')
    search_fields = ('icd10_code', 'description', 'encounter__patient__patient_number')
